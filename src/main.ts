...
import { AppManager , CalculatedVolumeSetter } from "./bim-components";
...
const calculatedVolumneSetter= components.get(CalculatedVolumeSetter)

const toolbar = BUI.Component.create(() => {
  
  const onComputeVolume=()=>{
    const selection=highlighter.selection.select
    if(Object.keys(selection).length===0) return;
    calculatedVolumneSetter.compute(world, selection)
